...
Nightwatch.prototype.setOptions = function(options) {
  ...
  var assertScreenshots = screenshots && screenshots.asserts || false;
  var commandScreenshots = screenshots && screenshots.commands || false;
  var assertScreenshotsEnabled = assertScreenshots && assertScreenshots.enabled || false;
  var commandScreenshotsEnabled = commandScreenshots && commandScreenshots.enabled || false;

  this.api.options.commandScreenshots = commandScreenshotsEnabled;
  if (commandScreenshotsEnabled) {
    if (typeof commandScreenshots.path == 'undefined') {
      throw new Error('Please specify the commandScreenshots.path in nightwatch.json.');
    }
    this.api.commandScreenshotsPath = this.api.options.commandScreenshotsPath = commandScreenshots.path;
  } else {
    this.options.commandScreenshots = {
      enabled : false,
      path : ''
    };
  }

  this.api.options.assertScreenshots = assertScreenshotsEnabled;
  if (assertScreenshotsEnabled) {
    if (typeof assertScreenshots.path == 'undefined') {
      throw new Error('Please specify the assertScreenshots.path in nightwatch.json.');
    }
    this.api.assertScreenshotsPath = this.api.options.assertScreenshotsPath = assertScreenshots.path;
  } else {
    this.options.assertScreenshots = {
      enabled : false,
      path : ''
    };
  }
...
Nightwatch.prototype.runProtocolAction = function(requestOptions, callback) {
  ...
  var request = new HttpRequest(requestOptions)
    ...
    .on('error', function(result, response, screenshotContent) {
      result = self.handleTestError(result);
      if (screenshotContent && self.options.screenshots.commands.enabled) {

        var fileNamePath = path.resolve(path.join(self.options.screenshots.commands.path, 'ERROR_' +
          Utils.getDateStamp()  + '.png'));
