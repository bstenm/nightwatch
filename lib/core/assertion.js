...
module.exports = new (function() {
  ...
  this.assert = function assert(passed, receivedValue, expectedValue, message, abortOnFailure) {
    ...
    (function (f) {
      var screenshots = client.options.screenshots && client.options.screenshots.asserts || {};
      if( !passed && screenshots.enabled ) {
        screenshotFilePath = path.resolve( path.join( screenshots.path, 'ERROR_' + Utils.getDateStamp() + '.png'));
        self.api.saveScreenshot( screenshotFilePath, f );
      } else {
        f();
      } 
    } (function () {
      var lastTest = client.results.tests[ client.results.tests.length-1 ];
      lastTest.screenshots = lastTest.screenshots || [];
      lastTest.screenshots.push( screenshotFilePath );
